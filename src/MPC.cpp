...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const double w_espi = 1.0;
    const double w_v = 1.0;
    const double w_delta = 1.0;
    const double w_a = 1.0;
    const double w_delta_diff = 1000.0;
    const double w_a_diff = 1000.0;
    //
    ...
    for (int i = 0; i < N; i++) {
      ...
      fg[0] += w_espi*CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
...
//
MPC::MPC() {
  result_x.resize(N-1); // for printing the green line
  result_y.resize(N-1); // for printing the green line
}
MPC::~MPC() {}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i=1; i<N; i++){
    result_x[i-1] = solution.x[i + x_start];
    result_y[i-1] = solution.x[i + y_start];
  }
